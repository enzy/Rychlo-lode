 ...
 *
 * @version @(#)Intro.java	1.6 99/08/16
...
public class Intro extends JPanel {
    ...
    static class Surface extends JPanel implements Runnable {
        ...
        static List list;                  // a cache for DdE part 
        ...
        /**
         * create a shuffled list for the DdE part - Dither Dissolve Effect
         */
        private void createShuffledList() {
            int size = bimg.getRaster().getDataBuffer().getSize();
        ...
        /**
         * Dither Dissolve Effect.  Render a shuffled list of pixels black
         * from the buffered image.
         *
         * The shuffled list is created in the Surface class 
         * createShuffledList() method.  
         */
        static class DdE implements Part {
            ...
            private int eNum;   // element number
            private int cSize;  // chunk size


            public DdE(int beg, int end) {
                this.beginning = beg;
                this.ending = end;
            }
            ...
            public void step(int w, int h) {
                if (bimg == null) {
                    ...
                    Graphics2D big = bimg.createGraphics();
                    big.drawImage(Surface.bimg, 0, 0, null);
                    db = bimg.getRaster().getDataBuffer();
                    cSize = list.size() / (ending - beginning);
                    eNum = -cSize;
                }
                eNum += cSize;
            }
            ...
            public void render(int w, int h, Graphics2D g2) {
                int black = 20 << 16 | 20 << 8 | 20;
                for (int j = eNum; j < eNum+cSize && j < list.size();j++) {
                    int val = ((Integer) list.get(j)).intValue();
                    db.setElem(val, black);
                }
                g2.drawImage(bimg, 0, 0, null);
            }
