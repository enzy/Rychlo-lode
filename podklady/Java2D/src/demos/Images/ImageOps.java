 ...
 */
public class ImageOps extends DemoSurface implements CustomControls, ChangeListener {
    ...
    public Component[] getCustomControls() {
        ...
        return new Component[] { (Component) controls, slider1, slider2 };
    }

    public void customControlsThread(int state) {
        if (state == CustomControls.START) {
            controls.start();
        } else if (state == CustomControls.STOP) {
            controls.stop();
        }
    }
    ...
    public static void main(String s[]) {
        JFrame f = new JFrame("Java2D Demo - ImageOps");
        f.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent e) {System.exit(0);}
        });
        f.getContentPane().add("Center", new DemoPanel(new ImageOps()));
        f.pack();
    ...
    static class DemoControls extends JPanel implements ActionListener, Runnable {
        ...
        public void start() {
            ...
            thread.setName("Images.ImageOps DemoControls Thread");
            thread.start();
        }
